...
import { NewsChatbotApiStack } from "./stacks/news-chatbot-api-stack";
...
function getDeployCase(app: App): DeployCase {
  ...
  throw new Error(
    `Invalid deploy case: ${deployCase}. Valid options are: ${Object.values(
      DeployCase
    ).join(", ")}`
  );
}
...
(async () => {
  ...
  const stackNamePrefix = "CSCGenAILab-";
  ...
  const commonStack = new CommonStack(app, stackNamePrefix + "CommonStack", {
    env: env,
    description:
      "CSC GENAILAB WORKSHOP (uksb-arjmsrldpz)(tag:" +
      deployCase +
      ", " +
      pace_deployment_version +
      ", " +
      deployment_name +
      ")",
  ...
  switch (deployCase) {
    ...
    case DeployCase.CHATBOT:
      ...
      const newsChatbotApiStack = new NewsChatbotApiStack(
        app,
        stackNamePrefix + "NewsChatbotApiStack",
        {
          env: env,
          commonStack: commonStack,
          knowledgeBaseId: "PGQV3JXPET", // 기존에 생성된 지식 기반 ID
        }
      );
      newsChatbotApiStack.addDependency(commonStack);
      break;
    case DeployCase.ALL:
